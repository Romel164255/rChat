...
export async function requestOTP(req, res) {
  try {
    ...
    if (isDev) {
      // Development: return the OTP in the response so you can test without an SMS provider.
      // In production set NODE_ENV=production and plug in Twilio / AWS SNS below.
      return res.json({ message: "OTP sent (dev mode)", otp });
    }

    // --- Production SMS (example: Twilio) ---
    // await twilioClient.messages.create({
    //   body: `Your Chatty OTP is: ${otp}`,
